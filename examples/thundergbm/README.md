...
-   For ROCm 6.4
   ...
   ```bash
  docker run --cap-add=SYS_PTRACE --ipc=host --privileged=true   --shm-size=128GB --network=host --device=/dev/kfd  --device=/dev/dri --group-add video -it   -v $HOME:$HOME --name rocm6 rocm/dev-ubuntu-24.04:6.4.4-complete
```
Install ThunderGBM prebuilt ROCm wheel:
```bash
# Example: adapt version numbers to your Python version
pip install thundergbm --index-url=pypi.amd.com/simple
```
